...
static int qvorbis_stream_output_samples16( qvorbis_stream_t *ogg_stream, int samples, int c, void *buffer ) {
	...
	clamp( samples, 0, ogg_stream->stb.frame_samples - ogg_stream->stb.frame_samples_offs );

	if( c == 1 ) {
		for( s = 0; s < samples; s++ ) {
			int l = (int)(scale * left[s]);
			shorts[0] = (short)bound( l, -32768, 32767 );
			shorts++;
		}
	} else {
		for( s = 0; s < samples; s++ ) {
			...
			shorts[0] = (short)bound( l, -32768, 32767 );
			shorts[1] = (short)bound( r, -32768, 32767 );
			shorts += 2;
		}
	}
...
int qvorbis_stream_read_samples( qvorbis_stream_t *ogg_stream, int samples, int c, int w, void* buffer ) {
	...
	for( r = 0; r < samples; ) {
		...
		if( used == 0 ) {
			const int chunk = bound( SND_OGG_MIN_PRELOAD_SAMPLES, samples - r, SND_OGG_MAX_PRELOAD_SAMPLES ) * bps;
